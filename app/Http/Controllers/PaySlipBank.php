...
class PaySlipBank extends Controller
{
	...
	public function checkPayslipListBankSlip(Request $request)
	{
		if ($request->ajax()) {
			...
			if ($request->location_filter_id == '-1') {
				return response()->json(['errors' => ['Select a Branch']]);
			}

			// ── Column / value defaults ───────────────────────────
			$emp_sal_held_col   = '0';
			$emp_location_col   = '1';
			$emp_department_col = '2';
			$emp_location_val   = '1';
			...
			$sal_epf_uploadno = substr('  ' . $request->salary_submit_attemptno, -2);

			if ($request->opt_rpt == '1') {
				if (!empty($request->location_filter_id)) {
					$emp_location_col = 'employees.emp_company';
					$emp_location_val = $request->location_filter_id;
				}
				if (!empty($request->department_filter_id)) {
					$emp_department_col = 'employees.emp_department';
			...
			// ── Bank detection ────────────────────────────────────
			// bank_id stored on companies table. Add more codes as needed:
			// 7083 = HNB | 7010 = BOC | 7278 = Sampath | 7021 = NTB
			$company_bank_id = $company_acc_info->bank_id ?? '7010'; // fallback HNB

			// ── Bank-specific company-level fields ────────────────
			// BOC: company name trimmed to 13 chars for file header
			...
			if (in_array($request->opt_rpt, ['1', '2', '3'])) {
				...
				drv_work.work_days,
				drv_info.fig_group_title, drv_info.fig_group,
				drv_info.fig_value AS fig_value, drv_info.epf_payable AS epf_payable,
				drv_info.remuneration_pssc
						FROM (SELECT employee_payslips.id AS emp_payslip_id,
									employees.id as employee_id,
									employees.emp_etfno AS emp_epfno,
									employees.emp_name_with_initial AS emp_first_name,
									employees.emp_national_id, employees.emp_last_name,
									employees.id as e_id,
									ifnull(job_titles.occupation_group_id, 0) as occupation_group_id,
									companies.name AS location,
									ebanks.bank_ac_no,
									ebanks.bank_code,
									ebanks.branch_code,
							...
							WHERE employee_payslips.payment_period_id=?
								...
								AND " . $emp_sal_held_col . "='0'
						) AS drv_emp
						INNER JOIN (SELECT `employee_payslip_id`, SUM(`work_days`) as work_days
									FROM `employee_paid_rates`
									WHERE concat(`salary_process_year`,`salary_process_month`)
										IN (date_format(?, '%Y%c'), date_format(?, '%Y%c'))
									GROUP BY employee_payslip_id
									HAVING work_days>0) as drv_work
							ON drv_emp.emp_payslip_id=drv_work.employee_payslip_id
						INNER JOIN (SELECT `id` AS fig_id,
										`employee_payslip_id`,
										`fig_group_title`, `fig_group`, `epf_payable`,
				...
				'ADDITION' AS fig_group_title, 'ADDITION' AS fig_group,
				drv_emp.payment_amount AS fig_value, 0 AS epf_payable, 'OTHER_REM' AS remuneration_pssc
						FROM (SELECT payroll_profiles.id AS emp_payslip_id,
									...
									ebanks.bank_code,
									ebanks.branch_code,
									employee_term_payments.payment_amount,
									0 AS payslip_held,
									1 AS payslip_approved
							FROM `employee_term_payments`
								INNER JOIN payroll_profiles ON employee_term_payments.payroll_profile_id=payroll_profiles.id
							...
							WHERE DATE_FORMAT(employee_term_payments.created_at,'%Y%m')=DATE_FORMAT(NOW(),'%Y%m')
								...
								AND " . $emp_location_col . "=?
								AND " . $emp_department_col . "=?
								AND employee_term_payments.payment_cancel=0
								AND " . $emp_sal_held_col . "='0'
						) AS drv_emp
						CROSS JOIN (SELECT 15 as work_days) as drv_work
				ORDER BY CAST(ifnull(nullif(drv_emp.disp_epfno,0),'99999') AS UNSIGNED), drv_emp.emp_epfno";
			...
			// ── Build employee_list (salary bank file rows) ───────
			$employee_list        = [];
			$cnt                  = 0;
			$act_payslip_id       = '';
			$net_payslip_fig_value = 0;
				...
				if (!empty($r->bank_ac_no)) {
					...
					if (!isset($employee_list[$cnt - 1])) {
						$employee_list[] = [
							...
							'emp_first_name'   => $r->emp_first_name,
							'location'         => $r->location,
							'BASIC'            => 0, 'NOPAY'    => 0, 'OTHRS'    => 0,
							'FACILITY'         => 0, 'LOAN'     => 0, 'ADDITION' => 0,
							'EPF8'             => 0, 'EPF12'    => 0, 'ETF3'     => 0,
							'PAYE'             => 0, 'NETSAL'   => 0,
							'payslip_cancel'   => 1,
							'payslip_held'     => $r->payslip_held,
							'payslip_approved' => $r->payslip_approved,
							'bank_format'      => $bank_format, // propagate to blade
						];
					}

					$employee_list[$cnt - 1][$r->fig_group_title] =
						number_format((float) $r->fig_value, 4, '.', '');

					if (!(($r->fig_group_title == 'EPF12') || ($r->fig_group_title == 'ETF3'))) {
						$net_payslip_fig_value += $r->fig_value;
						$employee_list[$cnt - 1]['NETSAL'] =
							number_format((float) $net_payslip_fig_value, 2, '.', '');

						// ── Account name (shared across all banks) ────
						$accountname = $r->emp_first_name;
						$accountname = str_replace(['.', 'Mrs', 'Mr', 'Miss'], [' ', '', '', ''], $accountname);
						$accountname = trim($accountname);
						$accountname = str_replace("'", '`', $accountname);
						$accountname = preg_replace("/(?![A-Z a-z 0-9])./", '', $accountname);
						$accountname = preg_replace('/\s+/', ' ', $accountname);
						...
						// ── Account number (shared across all banks) ──
						$accountno        = '000000000000';
						$accountno_intval = 0;
						if ($r->bank_ac_no != null) {
							$accountno        = preg_replace('/[-. ]/', '', $r->bank_ac_no);
							$accountno_intval = (int) ltrim($accountno, '0');
							$accountno        = substr('000000000000' . $accountno, -12);
						}

						// ══════════════════════════════════════════════
						// Per-bank file row — only ONE case executes.
						// Add new banks here when spec is received.
						// ══════════════════════════════════════════════
						switch ($company_bank_id) {
							...
							case '7083': // HNB
							// ────────────────────────────────────────
								// Amount: 11-digit zero-padded, no decimal point (HNB SLIPS spec)
								$hnb_amount = str_replace('.', '', number_format((float) $net_payslip_fig_value, 2, '.', ''));
								$hnb_amount = substr('00000000000' . $hnb_amount, -11);

								$ref_no       = substr('00000000' . $cnt, -8);
								$bank_br_code = $r->bank_code . $r->branch_code; // 7-digit

								$employee_list[$cnt - 1]['ref_no']       = $ref_no;       // 8 chars
								$employee_list[$cnt - 1]['emp_name']     = $accountname;  // 20 chars
								$employee_list[$cnt - 1]['bank_br_code'] = $bank_br_code; // 7 chars
								$employee_list[$cnt - 1]['account_no']   = $accountno;    // 12 chars
								$employee_list[$cnt - 1]['trx_code']     = '023';          // HNB detail trx code
								$employee_list[$cnt - 1]['amount']       = $hnb_amount;   // 11 chars
								$employee_list[$cnt - 1]['date']         = $formatted_date;
							...
							// ────────────────────────────────────────
							case '7278': // Sampath
							// TODO: populate sampath_* fields when spec received
							// ────────────────────────────────────────
								$trx_code = '';
								break;

							// ────────────────────────────────────────
							case '7021': // NTB (Nations Trust Bank)
							// TODO: populate ntb_* fields when spec received
							// ────────────────────────────────────────
								$trx_code = '';
								break;
							...
							// ────────────────────────────────────────
							default:
							// Unknown bank — store minimum fields so
							// the table still renders; download blocked in blade
							// ────────────────────────────────────────
								$hnb_amount = str_replace('.', '', number_format((float) $net_payslip_fig_value, 2, '.', ''));
								$hnb_amount = substr('00000000000' . $hnb_amount, -11);

								$employee_list[$cnt - 1]['ref_no']       = substr('00000000' . $cnt, -8);
								$employee_list[$cnt - 1]['emp_name']     = $accountname;
								$employee_list[$cnt - 1]['bank_br_code'] = $r->bank_code . $r->branch_code;
								$employee_list[$cnt - 1]['account_no']   = $accountno;
								$employee_list[$cnt - 1]['trx_code']     = '000';
								$employee_list[$cnt - 1]['amount']       = $hnb_amount;
								$employee_list[$cnt - 1]['date']         = $formatted_date;
						...
						$total_amount     += $r->fig_value;
						$hash_total       += ($accountno_intval - $accountno_preval);
						$accountno_preval  = $accountno_intval;
					}
				}
			}
			...
			// ── EPF / ETF lists ───────────────────────────────────
			$sum_array = [
				'emp_epfno' => '', 'emp_first_name' => '', 'emp_last_name' => '',
				'emp_nicnum' => '', 'occu_grade' => '', 'emp_workdays' => 0,
				...
				'EPF8' => 0, 'EPF12' => 0, 'ETF3' => 0,
				'sal_adv' => 0, 'ded_tp' => 0, 'ded_IOU' => 0, 'ded_fund_1' => 0,
				'ded_other' => 0, 'PAYE' => 0, 'LOAN' => 0,
				'tot_ded' => 0, 'NETSAL' => 0, 'OTHER_REM' => 0,
			];
			...
			$emp_fig_totlost  = 0; $emp_fig_otherlost = 0;
			$emp_fig_tottax   = 0;
			$rem_tot_bnp      = 0; $rem_tot_fortax    = 0;
			$rem_tot_earn     = 0; $rem_tot_ded       = 0;
			$rem_net_sal      = 0;
			...
			foreach ($employee as $r) {
				...
				if (!isset($emp_array[$cnt - 1])) {
					$emp_array[] = [
						...
						'emp_nicnum'    => $r->emp_national_id,
						'occu_grade'    => $r->occupation_group_id,
						'emp_workdays'  => $r->work_days,
						'BASIC' => 0, 'BRA_I' => '0', 'add_bra2' => '0', 'NOPAY' => 0,
						'tot_bnp' => 0, 'sal_arrears1' => 0, 'tot_fortax' => 0,
						'ATTBONUS' => 0, 'ATTBONUS_W' => 0, 'INCNTV_EMP' => 0, 'INCNTV_DIR' => 0,
						'add_transport' => 0, 'add_other' => 0, 'sal_arrears2' => 0,
					...
					$rem_tot_bnp   = 0; $rem_tot_fortax = 0;
					$rem_tot_earn  = 0; $rem_tot_ded    = 0;
					$rem_net_sal   = 0; $rem_ded_other  = 0;
				}

				$fig_key = isset($emp_array[$cnt - 1][$r->fig_group_title])
					? $r->fig_group_title
					: $r->remuneration_pssc;
				...
				if (isset($emp_array[$cnt - 1][$fig_key])) {
					...
					if (in_array($fig_key, ['BASIC', 'BRA_I', 'add_bra2'])) {
						$emp_tot_bnp = $emp_array[$cnt - 1]['BASIC']
			...
			// ── Build EPF / ETF formatted lists ───────────────────
			foreach ($emp_array as $epfetf_r) {
				if (($epfetf_r['EPF8'] + $epfetf_r['EPF12'] + $epfetf_r['ETF3']) > 0) {
					$pad_nicnum = substr(str_pad($epfetf_r['emp_nicnum'], 20, ' '), -20);
					$reg_nicnum = substr(str_pad('', 12, '0') . $epfetf_r['emp_nicnum'], -12);

					$accountname = $epfetf_r['emp_first_name'];
					$accountname = str_replace(['.', 'Mrs', 'Mr', 'Miss'], [' ', '', '', ''], $accountname);
					$accountname = str_replace("'", '`', $accountname);
					$accountname = preg_replace("/(?![A-Z a-z 0-9])./", '', $accountname);
					$accountname = preg_replace('/\s+/', ' ', $accountname);
					...
					$output_str  = str_replace(strtoupper(trim($epfetf_r['emp_last_name'])), '', strtoupper($accountname));
					$meminitials = substr(str_pad(strtoupper(trim($output_str)), 20, ' '), -20);
					$reglastname = strtoupper(trim($epfetf_r['emp_last_name']));
					$memlastname = substr(str_pad(trim($reglastname), 40, ' '), -40);
					$memsurname  = substr(str_pad(trim($reglastname), 30, ' '), -30);

					$pad_epfno      = substr(str_pad('', 6, '0') . $epfetf_r['emp_epfno'], -6);
					$emp_mem_totepf = ($epfetf_r['EPF8'] + $epfetf_r['EPF12']);
					$pad_totcont    = substr(str_pad('', 7, '0') . number_format((float) $emp_mem_totepf, 2, '.', ''), -10);
					$pad_mem_epf    = substr(str_pad('', 7, '0') . number_format((float) $epfetf_r['EPF8'], 2, '.', ''), -10);
					$pad_emp_epf    = substr(str_pad('', 7, '0') . number_format((float) $epfetf_r['EPF12'], 2, '.', ''), -10);
					$pad_totearn    = substr(str_pad('', 9, '0') . number_format((float) $epfetf_r['tot_fortax'], 2, '.', ''), -12);
					$fig_workdays   = $request->pay_schedule_days;
					$pad_workdays   = substr(str_pad('', 2, '0') . number_format((float) $fig_workdays, 2, '.', ''), -5);
					$pad_mem_etf    = substr(str_pad('', 9, '0') . number_format((float) $epfetf_r['ETF3'], 2, '', ''), -9);
					$occu_grade     = substr(str_pad('', 3, ' ') . $epfetf_r['occu_grade'], -3);
					...
					$employee_epf_list[] = [
						'emp_epfno'      => $npad_epfno,
						'emp_first_name' => $meminitials,
						'emp_last_name'  => $memlastname,
						'emp_nicnum'     => $pad_nicnum,
						'emp_workdays'   => $npad_workdays,
						'tot_fortax'     => $npad_totearn,
						'EPF8'           => $npad_mem_epf,
						'EPF12'          => $npad_emp_epf,
						'tot_epf'        => $npad_totcont,
						'member_status'  => 'E',
						'zone_code'      => $txt_zone_code,
						'employer_num'   => $npad_employer_num,
						'cont_period'    => $cont_period_fr,
						'submit_num'     => $sal_epf_uploadno,
						'occu_grade'     => $occu_grade,
					];

					$employee_etf_list[] = [
						'etf_rec_code'   => 'D',
						'employer_num'   => $txt_employer_no,
						'emp_etfno'      => $pad_epfno,
						'emp_first_name' => $meminitials,
						'emp_last_name'  => $memsurname,
						'emp_nicnum'     => $reg_nicnum,
						'ETF3'           => $pad_mem_etf,
						'cont_period_fr' => $cont_period_fr,
						'cont_period_to' => $cont_period_to,
					];
				}
			}
			...
			return response()->json([
				'employee_detail'     => $employee_list,
				'ei'                  => $ei,
				'total_amount'        => $header_tot_payment,
				'date_e'              => $formatted_date,
				'hash_total'          => $header_hash_total,
				'trx_code'            => $trx_code,
				'no_of_transactions'  => $no_of_transactions,
				'payment_period_id'   => $payment_period_id,
				'work_date_fr'        => $payment_period_fr,
				...
				'acc_name'            => $txt_acc_name,
				'acc_number'          => $txt_acc_number,
				'acc_br_code'         => $txt_acc_br,
				'bank_id'             => $company_bank_id,  // e.g. '7083', '7010'
				'bank_format'         => $bank_format,       // 'HNB', 'BOC', 'SAMPATH', etc.
			]);
		}
	}
