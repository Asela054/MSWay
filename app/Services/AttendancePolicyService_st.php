...
class AttendancePolicyService
{
    ...
    { 
         $empshift = DB::table('employees')
            ...
            $employeeshiftdetails = DB::table('employeeshiftdetails')
               ...
               $intime = Carbon::parse($date_input . ' ' . $timestamp);
                $rangeStart = Carbon::parse($date_input . ' 07:00:00');
                $rangeEnd   = Carbon::parse($date_input . ' 07:30:00');
                $timestamp  = $intime->between($rangeStart, $rangeEnd)
    ...
    {  
      ...
      $previousDate = Carbon::parse($date_stamp)->subDay()->format('Y-m-d');
        ...
        $intime = Carbon::parse($date_stamp . ' ' . $time_string);
        $rangeStart = Carbon::parse($date_stamp . ' 07:00:00');
        $rangeEnd   = Carbon::parse($date_stamp . ' 07:30:00');
        if ($intime->between($rangeStart, $rangeEnd)) {
            $time_h = '07';
            $time_m = '30';
        }
