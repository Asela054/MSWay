            ...
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content">
                    <div class="modal-header p-2">
                        <h5 class="modal-title" id="staticBackdropLabel">Add Production Amount</h5>
                                ...
                                <form method="post" id="formTitle" class="form-horizontal">
                                    ...
                                    <div class="form-row mb-2">
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <select class="form-control form-control-sm" name="jobtitle" id="jobtitle">
                                    ...
                                    <div class="form-row mb-2">
                                        <div class="col-md-6">
                                            <label class="small font-weight-bold text-dark">End Percentage*</label>
                                            <input type="number" name="end_precentage" id="end_precentage" class="form-control form-control-sm" step="0.01" placeholder="End Percentage" required>
                                        </div>
                                        <div class="col-md-6">
...
$(document).ready(function () {
    ...
    $('#dataTable').DataTable({
        ...
        ajax: {
            url: scripturl + '/Opma_Production/production_amount_list.php',
            type: "POST",
            data: {},
        },
        columns: [
                ...
                { data: 'end_precentage', name: 'end_precentage' },
    ...
    $('#create_record').click(function(){
        $('.modal-title').text('Add Production Amount');
    ...
    $('#formTitle').on('submit', function(event){
        ...
        if ($('#action').val() == 'Add') {
            action_url = "{{ route('addopma_production_amount') }}";
        }
        if ($('#action').val() == 'Edit') {
            action_url = "{{ route('opma_production_amount.update') }}";
        }
    ...
    $(document).on('click', '.edit', async function () {
        ...
        $.ajax({
            url: "opma_production_amount/" + id + "/edit",
            dataType: "json",
            success: function (data) {
                $('#department').append($('<option>', {
                    value: data.result.department_id,
                    text: data.result.dept_name,
                    selected: true
                })).trigger('change');
                $('#jobtitle').append($('<option>', {
                    value: data.result.job_title_id,
                    text: data.result.job_title,
                    selected: true
                })).trigger('change');
                $('#end_precentage').val(data.result.end_precentage);
                $('#amount').val(data.result.amount);

                $('#hidden_id').val(id);
                $('.modal-title').text('Edit Production Amount');
    ...
    $(document).on('click', '.delete', async function () {
        ...
        if (r == true) {
            ...
            $.ajax({
                url: "{{ url('opma_production_amount/destroy/') }}/" + user_id,
                beforeSend: function () {
