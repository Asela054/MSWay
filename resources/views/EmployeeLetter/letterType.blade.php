                        ...
                        <div class="col">
                            ...
                            <form method="post" id="formTitle" class="form-horizontal">
                                ...
                                <div class="form-group mb-1">
                                    <label class="small font-weight-bold text-dark">Letter Type <span class="text-danger">*</span></label>
                                    <input type="text" name="letter_type" id="letter_type" class="form-control form-control-sm" required />
                                </div>
                                <div class="form-group mb-1">
...
<script>
    $(document).ready(function() {
        ...
        $('#dataTable').DataTable({
            ...
            ajax: {
                url: scripturl + "/EmployeeLetter/letterType_list.php",
                type: "POST",
                data: {},
            },
            columns: [{
        ...
        $('#create_record').click(function() {
            $('.modal-title').text('Add Letter type');
        ...
        $('#formTitle').on('submit', function(event) {
            ...
            if ($('#action').val() == 'Add') {
                action_url = "{{ route('addLetterType') }}";
            }

            if ($('#action').val() == 'Edit') {
                action_url = "{{ route('letterType.update') }}";
            }
        ...
        $(document).on('click', '.edit', async function() {
            ...
            if (r == true) {
                ...
                $('#form_result').html('');
                $.ajax({
                    url: "letterType/" + id + "/edit",
                    dataType: "json",
                    success: function(data) {
                        $('#letter_type').val(data.result.letter_type);
                        $('#remarks').val(data.result.remarks);
                        $('#hidden_id').val(id);
                        $('.modal-title').text('Edit Letter type');
                        $('#action_button').html('<i class="fas fa-check"></i>&nbsp;Update');
                        $('#action').val('Edit');
                        $('#formModal').modal('show');
                    },
                    error: function(xhr, status, error) {
        ...
        $(document).on('click', '.delete', async function() {
            ...
            if (r == true) {
                ...
                $.ajax({
                    url: "{{ url('letterType/destroy') }}/" + user_id,
                    beforeSend: function() {
