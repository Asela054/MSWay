                    ...
                    <div class="col-12">
                        ...
                        <table class="table table-striped table-bordered table-sm small nowrap" style="width: 100%" id="jobtable">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{$ad->device_name}}</td>
                                    <td>{{$ad->remarks}}</td>
                                    <td class="text-right">
                                            <button name="edit" id="{{$ad->id}}" class="edit btn btn-primary btn-sm" type="submit"><i class="fas fa-pencil-alt"></i></button>
                                            <button type="submit" name="delete" id="{{$ad->id}}" class="delete btn btn-danger btn-sm"><i class="far fa-trash-alt"></i></button>
                                    </td>
                                </tr>
                                @endforeach
                            </tbody>
                            ...
                            <form method="post" id="formTitle" class="form-horizontal">
                                ...
                                <div class="form-row">
                                    <div class="col-sm-4 col-md-4">
                                        <label class="small font-weight-bold text-dark">Device Name*</label>
                                        <input type="text" name="device_name" id="device_name" class="form-control form-control-sm"  required/>
                                    </div>
...
$(document).ready(function(){
    ...
    $('#formTitle').on('submit', function (event) {
        ...
        if ($('#action').val() == 'Add') {
            action_url = "{{ route('addDevices') }}";
        }
        if ($('#action').val() == 'Edit') {
            action_url = "{{ route('Devices.update') }}";
        }
    ...
    $(document).on('click', '.edit', async function () {
        ...
        $.ajax({
            ...
            dataType: "json",
            success: function (data) {
                $('#device_name').val(data.result.device_name);
